...
export default function QueryPanel({ documentId }: QueryPanelProps) {
  ...
  // Highlight sources as soon as they arrive (before answer finishes)
  const hasSetHighlights = sources.length > 0;
  if (hasSetHighlights) {
    const chunkIds = sources.map((s) => s.chunk_id);
    // We call this on every render when sources change — React dedupes
